 ...
 */
class DftMatrixPrimeLowerPartComplexDouble(override val root: RootP<ComplexNumber<Double>>) : DftMatrixPrimeLowerPart<ComplexNumber<Double>> {
    ...
    init {
        ...
        dftInv = dft.inverse
        val rgInv: List<ComplexNumber<Double>> = list(root.order.value - 1u) { i -> root.root.cachedPower(g.root.cachedInversePower(i).residue) }
    ...
    override fun timesImpl(matrix: AbstractMatrix<ComplexNumber<Double>>): AbstractMatrix<ComplexNumber<Double>> {
        val mgList = list(rows) { i -> matrix.rowListAt(g.root.cachedPower(i).residue) }
