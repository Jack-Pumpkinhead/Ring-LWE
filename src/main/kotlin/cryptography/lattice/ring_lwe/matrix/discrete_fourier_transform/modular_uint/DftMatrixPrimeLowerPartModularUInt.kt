 ...
 */
class DftMatrixPrimeLowerPartModularUInt(root: RootDataUIntPrime<ModularUInt>) : DftMatrixPrimeLowerPart<ModularUInt>(root) {
    ...
    init {
        ...
        dftInv = dft.inverse()
        val rgInv: List<ComplexNumber<Double>> = list(root.order.value - 1u) { i -> FieldDouble.realComplexNumber(root.power(g.inversePower(i).residue).residue.toDouble()) }
    ...
    override fun timesImpl(matrix: AbstractMatrix<ModularUInt>): AbstractMatrix<ModularUInt> {
        val mgList = list(rows) { i -> matrix.rowListAt(g.power(i).residue).map { FieldDouble.realComplexNumber(it.residue.toDouble()) } }
