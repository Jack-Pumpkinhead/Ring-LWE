 ...
 */
class RootDataULong<A>(val ring: Ring<A>, val root: A, val order: ULong, val orderFactorization: List<PrimePowerULong>) {

    fun eulerTotient(): ULong = RingULong.product(orderFactorization.map { it.eulerTotient() })

    fun subRootDataPrimePower(primeIndex: UInt): RootDataULong<A> {
        val factor = orderFactorization[primeIndex.toInt()]
        val subRoot = ring.powerM(root, order / factor.primePower)
        return RootDataULong(ring, subRoot, factor.primePower, listOf(factor))
    }

    fun subRootDataPrime(primeIndex: UInt): RootDataULong<A> {
        val factor = orderFactorization[primeIndex.toInt()]
        var power = 1uL
        ...
        return RootDataULong(ring, ring.powerM(root, power), factor.prime, listOf(PrimePowerULong(factor.prime, 1u, factor.prime)))
    }

    fun allSubRootDataPrimePower(): List<RootDataULong<A>> {
        return orderFactorization.indices.map { i -> subRootDataPrimePower(i.toUInt()) }
    }

    fun subRootData(decreaseIndex: UInt): RootDataULong<A> {
        ...
        return RootDataULong(ring, ring.powerM(root, factor.prime), order / factor.prime, orderFactorization1)
    }

    /**
     * if decreaseOrder is maximal, return 1 with order 1
     */
    fun subRootData(decreaseOrder: List<UInt>): RootDataULong<A> {
        ...
        return RootDataULong(ring, ring.powerM(root, totalDividend), order / totalDividend, orderFactorization1)
    }
