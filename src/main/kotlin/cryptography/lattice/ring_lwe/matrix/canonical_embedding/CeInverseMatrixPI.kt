 ...
 */
class CeInverseMatrixPI<A>(override val inverse: CeMatrixPI<A>, dftBuilder: DftMatrixPPIBuilder<A>) : AbstractSquareMatrix<A> {
    ...
    override fun elementAtUnsafe(row: UInt, column: UInt): A {
        return ring.multiply(inverseP, ring.subtract(inverse.root.cachedInversePower(row * (column + 1u)), inverse.root.cachedPower(column + 1u)))
    }
