...
/**
 * Created by CowardlyLion at 2022/3/5 14:54
 */
class ComplexFieldCePPITestBase(val builder: CeMatrixBuilder<ComplexNumber<Double>>, val statistic: TaskNearMatrixComplexDoubleStatistic<TwoMatrix<ComplexNumber<Double>>> = TaskNearMatrixComplexDoubleStatistic(TwoMatrixMultiplicationTiming())) {
                ...
                is RootPPP -> {
                    for (subroot in SubrootCalculatorUnsafeComplexDouble.allMaximalPrimePowerSubroot(root)) {
                        ...
                        util.stdlib.repeat(repeat) {
                            val x = FieldComplexNumberDouble.randomMatrix(ce.columns, 2u, subroot.order.value.toDouble())
